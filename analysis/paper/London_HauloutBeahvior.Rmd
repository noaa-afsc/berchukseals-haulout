...
species_colors <- setNames(met.brewer("Isfahan2",n=3,direction=-1,override.order = TRUE),
                           c("Bearded seal","Ribbon seal","Spotted seal"))
...
p <- ggplot() +
  layer_spatial(survey_area, color = "#ddc000", 
                fill = "#ddc000", 
                alpha = 0.15,
                linewidth = 0.5) +
  ...
  annotation_spatial(world,
                     linewidth=0,
                     fill = "grey80") +
  geom_sf_text(data = label_coords_bering, aes(label = "Bering sea"), size = 1.75, color = "grey40") +
  geom_sf_text(data = label_coords_chukchi, aes(label = "Chuckchi \nsea"), size = 1.75, color = "grey40") +
  geom_sf_text(data = label_coords_beaufort, aes(label = "Beaufort \nsea"), size = 1.75, color = "grey40") +
  facet_grid(cols = vars(species), labeller = species_labels,
             switch="x") +
  scale_fill_manual(values = met.brewer("Isfahan2",n=3,direction=-1,override.order = TRUE),
                    aesthetics = "fill", 
                    name = "species") + 
  ggtitle("Initial bio-logger deployment locations by species",
          subtitle = 
  ...
  labs(caption = "shaded yellow region indicates spatial extent of aerial survey effort in US waters") +
  theme(legend.position = "none",
        axis.title = element_blank(),
        strip.background = element_blank(),
        strip.placement = "outside",
        strip.text.x = element_markdown(face = "bold"),
        plot.subtitle = element_textbox_simple(margin = ggplot2::margin(b=12)))
...
p <- p + theme(legend.position = "bottom") +
  ...
  ggtitle("Example percent-dry actogram from bio-logger data",
          subtitle = str_wrap("Observation records shown are from a single ribbon seal across a two year deployment. Note that not all records are received from the device.", width = 85)) +
...
(ref:mapCaption) Spatial distribution of haul-out records during the months of March through July 15 for each of the three species. Data were collated across all years between 2005 and 2020. Each hexagonal cell represents an area of 50 km^2^
...
ggplot(data = seal_hours_tbl, aes(x = yday, y = n, fill = n))+
  ...
  ggtitle("Density of haul-out behavior observations by species",
          subtitle = str_wrap("data are grouped by day-of-year and presented in seal-hours collated across years (2005-2020) from March through July"))
...
```{r beardedHOCal, include=TRUE, fig.cap="Predicted hourly haul-out probability of bearded seals (all ages and sex classes) from 1 March to 15 July for all and sex classes."}
...
p <- p + theme(legend.position = "bottom") +
  ...
  ggtitle("Bearded seal predicted haul-out probability",
          subtitle = str_wrap("Periods of highest haul-out probability for bearded seals appear to be centered around solar noon and midnight"))
p
```
...
p <- p + theme(legend.position = "bottom") +
  ...
  ggtitle("Ribbon seal predicted haul-out probability.",
          subtitle = str_wrap("Periods of highest haul-out probability for ribbon seals appear centered around solar noon and age-sex classes show differing seasonal patterns."))
p
```
...
```{r ribbonHOwx, fig.asp = 0.9, include=TRUE, fig.cap="Marginal effects of temperature, wind, pressure, and precipitation on the predicted haul-out probability of ribbon seals within each age and sex classification. Hour of the day was fixed at local solar noon and day-of-year held at the average day of peak haul-out for ribbon seals. Vertical lines represent the 95\\% confidence interval around the predicted haul-out probability."}
...
p <- p + theme(legend.position = "bottom") +
  ...
  ggtitle("Spotted seal predicted haul-out probability.",
          subtitle = str_wrap("Periods of highest haul-out probability for spotted seals appear centered around solar noon and age-sex classes show differing seasonal patterns."))
p
```
...
institutions responsible for the bio-logger deployments used in this study.
...
deploy_tbl <- deploy_tbl %>% 
  ...
  tab_footnote(footnote = "ADF&G = Alaska Department of Fish and Game",
               locations = cells_body(columns = agency, 
                                      rows = agency == "ADFG")) %>%
  tab_footnote(footnote = "NSB = North Slope Borough",
               locations = cells_body(columns = agency, 
                                      rows = agency == "NSB")) %>%
  tab_footnote(footnote = "NMFS = NOAA National Marine Fisheries Service",
               locations = cells_body(columns = agency, 
                                      rows = agency == "NMFS")) %>%
  tab_footnote(footnote = "NPWC = North Pacific Wildlife Consortium",
               locations = cells_body(columns = agency, 
                                      rows = agency == "NPWC"))
