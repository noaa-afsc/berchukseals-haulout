...
deployed, hourly percent dry observations from the flipper tag were preferred.
Sex and age class (non-dependent *young-of-the-year*, sexually immature
*subadults*, and mature *adults*) were determined at the time of deployment by
varying combinations of length, claw annuli [@burns1969; @burns1983;
@mclaren1958a], and pelage characteristics for some species. Field
...
sexually mature and older than four years. For those bio-loggers deployed on
...
\@ref(fig:dataCal) shows a calendar-like distribution of all haul-out data
...
ggplot(data = seal_hours_tbl) + 
  ...
  scale_size(range = c(1,3.4), breaks = c(100,500,1000,1500)) +
...
and molting period, however, hourly percent-dry data for days during a
deployment were not successfully transmitted. This is due to a variety of
factors including satellite coverage, tag availability (i.e. tags mounted to the
rear flipper are not available to the surface while the seal is at sea), tag
performance, duty cycling, and extra-terrestrial atmospheric interference.
...
weather covariates that vary with space and time. We explored the use of a
...
relationship between haul-out behavior and weather covariates and their
...
We also investigated the influence of weather on haul-out
...
and may also provide an optimal situation for predator detection. These results
