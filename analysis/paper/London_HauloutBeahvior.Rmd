...
abstract: >
  ...
  pupping, breeding, molting, and resting. In the Arctic, many of these
  activities occur in spring (April -- June) as sea ice begins to melt and
  retreat northward. Rapid acceleration of climate change in Arctic ecosystems
  is therefore of concern as the quantity and quality of suitable habitat is
  forecast to decrease. Improved estimates of seal population abundance are
  needed to properly monitor the impacts of these changes over time. In this
  paper, we use hourly percent-dry data from satellite-linked bio-loggers
  deployed between 2005 and 2021 to quantify the proportion of seals hauled out
  on ice. This information is needed to accurately estimate abundance from
  aerial survey counts of ice-associated seals (i.e., to correct for the
  proportion of animals that are in the water while surveys are conducted). In
  addition to providing essential data for survey 'availability' calculations,
  our analysis also provides insights into the seasonal timing and environmental
  ...
  snow lairs analysis of their 'availability' to surveys requires special
  consideration; therefore,they were not included in this analysis. Using
  generalized linear mixed pseudo-models to properly account for temporal
  autocorrelation, we fit models with covariates of interest (e.g., day-of-year,
  solar hour, age-sex class, wind speed, barometric pressure, temperature,
  precipitation) to examine their ability to explain variation in haul-out
  probability. We found evidence for strong diel and within-season patterns in
  haul-out behavior, as well as strong weather effects (particularly wind and
  temperature). In general, seals were more likely to haul out on ice in the
  middle of the day and when wind speed was low and temperatures were higher.
  Haul-out probability increased through March and April, peaking in May and
  early June before declining again. The timing and frequency of haul-out events
  also varied based on species and age-sex class. For ribbon and spotted seals,
  models with year effects were highly supported, indicating that the timing and
  magnitude of haul-out behavior varied among years. However, we did not find
  broad evidence that haul-out timing was linked to annual sea ice extent. Our
  analysis emphasizes the importance of accounting for seasonal and temporal
  variation in haul-out behavior, as well as associated environmental
  covariates, when interpreting the number of seals counted in aerial surveys.
output:
  bookdown::pdf_book:
...
aerial surveys (e.g.,
@bengtson2005a, @conn2014a, and @verhoef2014a) and more recent efforts have
...
aircraft [@conn2014a; @verhoef2014a]. Refining these inferences will improve the
...
How ice-associated seals use sea ice as a haul-out platform varies between
species. Ribbon seals (_Histriophoca fasciata_) haul out of the water almost
exclusively on sea ice and are mostly pelagic outside the spring pupping,
breeding, and molting season [@boveng2018a]. Although spotted (_Phoca largha_)
and bearded (_Erignathus barbatus_) seals occasionally rest on coastal land,
they primarily use sea ice as a haul-out platform during the spring and early
summer [@frost2018]. Ringed seals (_Phoca hispida_) --- not included in this
...
shown to influence walrus haul-out behavior [@udevitz2009] and we expect weather
...
Material, [S1](#s1)). Data from additional
...
species_labels <- c("Bearded seal" = paste0("<span style = 'color:",
                                            ...
                                            ";'>Bearded seal<br />Deployments</span>"),
                    "Ribbon seal" = paste0("<span style = 'color:",
                                           species_colors['Ribbon seal'],
                                           ";'>Ribbon seal<br />Deployments</span>"),
                    "Spotted seal" = paste0("<span style = 'color:",
                                            species_colors['Spotted seal'],
                                            ";'>Spotted seal<br />Deployments</span>")
) %>% as_labeller()

label_coords_bering <- st_sfc(st_point(c(178.5,57)), crs = 4326) %>%
  st_transform(st_crs(deploy_sf))
label_coords_chukchi <- st_sfc(st_point(c(-175,74.5)), crs = 4326) %>%
  st_transform(st_crs(deploy_sf))
label_coords_beaufort <- st_sfc(st_point(c(-150,72.25)), crs = 4326) %>%
  st_transform(st_crs(deploy_sf))

p <- ggplot() +
  annotation_spatial(survey_area, color = "#A26E7C", 
                fill = "#A26E7C", 
                alpha = 0.15,
                linewidth = 0.35) +
  ...
  annotation_spatial(world,
                     linewidth=0,
                     fill = "grey80") +
  geom_sf_text(data = label_coords_bering, aes(label = "Bering sea"), size = 2, color = "grey30") +
  geom_sf_text(data = label_coords_chukchi, 
               aes(label = "Chuckchi \nsea"), size = 2, lineheight = .85, color = "grey30") +
  ...
  scale_fill_manual(values = species_colors,
                    aesthetics = "fill", 
                    name = "species") + 
  labs(caption = "<span style = 'color: #A26E7C;'><span style = 'font-family: \"Font Awesome 6 Free Regular\";'>&#xf0c8;</span> spatial extent of aerial survey effort in US and Russian waters</span>") +
...
Sex and age class (non-dependent _young-of-the-year_, sexually immature
...
transmitting into the next year, the age class was advanced to subadult on 1
March of the following year -- the assumed anniversary of their birth. Subadults
are those seals likely greater than one year of age but less than four years.
Adults are individuals that are likely older than four years. Table
\@ref(tab:tblDeploy) provides a summary of these deployments and data received
from them.

(ref:deployTblCaption) **Summary of bio-logger data across seal species and age classification from 1 March to 15 July 2005-2021.** \linebreak Total seal hours represents the sum of hourly observations across all seals used in the analysis. Because young-of-the-year are advanced to subadult on 1 March of the following year, some individual seals are represented in both columns in this table

```{r tblDeploy, include=TRUE, tab.cap = "(ref:deployTblCaption)"}
...
(ref:examplePlotCaption) **Example percent-dry actogram from bio-logger data**
...
during a deployment are reliably transmitted from the bio-logger and since data
...
are represented as empty rectangles.

```{r examplePlot, include=TRUE, fig.id="examplePlot", fig.cap="(ref:examplePlotCaption)"}
...
p <- ggplot(d,aes(day,hour,fill=percent_dry))+
  ...
  scico::scale_fill_scico(palette = "nuuk", direction = 1,labels = scales::percent_format(scale=1),
                       name = "Example hourly percent dry data from a ribbon seal deployment", aesthetics = "fill",
...
in the Bering Sea [@conn2014a].

(ref:kgm2) kg/m^2^

(ref:tblCovariatesCaption) **Explanatory covariates used in analyses of binary haul-out records for bearded, spotted, and ribbon seals.** \linebreak Note that we also considered select interactions (see article text) between these primary covariates.  For instance, wind chill was represented by the interaction temperature:wind.

```{r tblCovariates, include=TRUE, tab.cap = "(ref:tblCovariatesCaption)"}
...
(GLMPMs; [@verhoef2010a]) to model variation in haul-out behavior as a function
of (1) covariate predictors, (2) temporally autocorrelated random effects, and
(3) individual random effects representing heterogeneity in individual behavior.
We used the glmmLDTS package [@verhoef2010a] to implement GLMPMs. We explored
two different model formulations for our data and we fit separate models to
bearded, ribbon, and spotted seal data sets as we expected differing behavior by
species. Separate models for each species were also needed because a single,
very large data set proved computationally intractable. In our first model
formulation and for each species, we fitted a year-independent model that
predicted average haul-out behavior as a function of demographic, weather,
seasonal, and diurnal effects. Second, for ribbon and spotted seals (which had
considerably more data than bearded seals), we fitted models that included all
the effects from the first model, but also permitted annual variation in
haul-out timing. This second set of models was used to examine whether
haul-out patterns varied by year and to determine the annual timing of apparent
peaks in haul-out behavior. For both models, we assumed an hourly Bernoulli
response (a binary 0-1 response dependent upon whether the tag was greater than 
50% dry for a given hour) where the linear predictor was 
modeled on the logit scale. This is
consistent with previous approaches [@verhoef2014a, @london2012] and only
...
(ref:mapCaption) **Spatial distribution of haul-out records from 1 March through July 15 for bearded, ribbon, and spotted seal.** \linebreak Linking location estimates with haul-out records in space and time allows for inclusion of weather covariates in the final model. For this visualization, data were collated across all years between 2005 and 2020 and each hexagonal cell represents an area of 50 km^2^

```{r dataMap, include=TRUE, fig.asp = 0.5, fig.cap="(ref:mapCaption)"}
...
ggplot() +
  ...
  scico::scale_fill_scico(
    ...
    trans = "log10",
    aesthetics = "fill",
    name = "number of haul-out records",
...
ggplot(data = seal_hours_tbl, aes(x = yday, y = n, fill = n))+
  ...
  guides(fill = guide_colorbar(title.position = "top", 
                               title.hjust = 0, barwidth = 15,
                               barheight = 0.5)) +
  facet_wrap(species ~ ., nrow=3) + 
  labs(fill = "Seal-hours of haul-out behavior data") +
  theme(axis.text.x = element_text(hjust = 0),
        strip.text.x = element_text(hjust = 1, margin=margin(r=15)),
...
increasing transmission rates (see @boveng2013)). All data after the first
...
(ref:beardedHOCalCaption) **Bearded seal predicted haul-out probability** \linebreak Predicted hourly haul-out probability of bearded seals (all ages and sex classes) from 15 March to 30 June for all age and sex classes combined. Predictions in July and before 15 March were not included to avoid spurious model predictions at the edge of the data range. Weather covariate values in the prediction were based on a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season. Note, our model predicts lower haul-out probability for bearded seals overall compared to ribbon and spotted seals and the color scales are not directly comparable.

```{r beardedHOCal, include=TRUE, fig.cap="(ref:beardedHOCalCaption)"}
...
p <- ggplot(plot_df, aes(day,solar_hour,fill = ho_prob, color = ho_prob)) +
  ...
  scico::scale_fill_scico(palette = "navia", direction = 1,
                       name = "Bearded seal haul-out probability", 
                       guide = guide_colorbar(title.position = "top", barwidth = 15,
                                              barheight = 0.5, title.hjust = 0)
  ) +
...
p <- ggplot(plot_df, aes(day,solar_hour,fill = ho_prob, color = ho_prob)) +
  ...
  scico::scale_fill_scico(palette = "navia", direction = 1,
                          begin = 0.1, end = 1,
                       name = "Ribon seal haul-out probability", 
...
(ref:ribbonHOwxCaption) **Influence of weather covariates on ribbon seal haul-out probability** \linebreak Marginal effects of temperature, wind, barometric pressure, and precipitation on the predicted haul-out probability of ribbon seals within each age and sex classifications. Hour of the day was fixed at local solar noon and day-of-year fixed at 15 May. The other weather covariate values were predicted from a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season. While the marginal effect of the covariate is continuous, points and vertical lines representing the 95% confidence interval around the predicted haul-out probability are shown only at observed weather values to give an indication of how the observations are distributed across the range of weather values.

```{r ribbonHOwx, fig.asp = 0.9, include=TRUE, fig.cap="(ref:ribbonHOwxCaption)"}
...
(ref:spottedHOCalCaption) **Spotted seal predicted haul-out probability** \linebreak Predicted hourly haul-out probability of spotted seals from 1 March to 30 June for each age and sex class used in the model. Weather covariate values in the prediction were based on a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season. As with ribbon seals, predictions for young of the year show their transition from newly weaned pups resting on the ice to more in-water activities. Predictions in July and before 15 March were not included to avoid spurious model predictions at the edge of the data range.

```{r spottedHOCal, fig.asp = 0.9, include=TRUE, fig.cap="(ref:spottedHOCalCaption)"}
...
p <- ggplot(plot_df, aes(day,solar_hour,fill = ho_prob, color = ho_prob)) +
  ...
  scico::scale_fill_scico(palette = "navia", direction = 1,
                          begin = 0.1, end = 1,
                       name = "Spotted seal haul-out probability", 
                       limits = c(0,1), breaks = c(0.25, 0.50, 0.75),
                       guide = guide_colorbar(title.position = "top", barwidth = 15,
                                              barheight = 0.5, title.hjust = 0)
  ) +
...
`r spotted_type3$pressure$Prob.F`) were all significant. Spotted seals were more likely to be on the ice when
...
(ref:spottedHOwxCaption) **Influence of weather covariates on spotted seal haul-out probability** \linebreak Marginal effects of temperature, wind, barometric pressure, and precipitation on the predicted haul-out probability of spotted seals within each age and sex classification. Hour of the day was fixed at local solar noon and day-of-year fixed at 15 May. The other weather covariate values were predicted from a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season.. While the marginal effect of the covariate is continuous, points and vertical lines representing the 95% confidence interval around the predicted haul-out probability are shown only at observed weather values to give an indication of how the observations are distributed across the range of weather values.

```{r spottedHOwx,fig.asp = 0.9, include=TRUE, fig.cap="(ref:spottedHOwxCaption)"}
...
$F_{`r paste(spotted_year_type3$day_2_year$Num.df, spotted_year_type3$day_2_year$Den.df, sep=",")`}$
= `r spotted_year_type3$day_2_year$F.value`; $p$ =
`r spotted_year_type3$day_2_year$Prob.F`). Ribbon seals showed no significant
...
$F_{`r paste(ribbon_year_type3$day_2_year$Num.df, ribbon_year_type3$day_2_year$Den.df, sep=",")`}$
= `r ribbon_year_type3$day_2_year$F.value`; $p$ =
`r ribbon_year_type3$day_2_year$Prob.F`). Predicted distributions of
...
(ref:annualHOcaption) **Annual variability in the timing of peak haul-out probability for ribbon and spotted seals.** \linebreak Spring haul-out probability (colored markers) for ribbon and spotted seals across 14 years are shown. The full seasonal range of annual predictions along with uncertainty are presented as grey lines. Covariates were fixed at local solar noon and under smoothed weather conditions. Only those groups (age:sex + year) that included observations from more than one seal are shown. Additionally, any groups where data were only available after 1 June or before 1 May are not included.

```{r annualHO, fig.asp = 0.75, include=TRUE, fig.cap="(ref:annualHOcaption)"}
...
peak_ho_df <- peak_ho_df %>% 
  ...
  mutate(year = forcats::fct_relevel(.$year, sort)) %>%
  mutate(year_shape = if_else(as.numeric(paste(year)) < 2015,"a","b"))


pal <- c(rcartocolor::carto_pal(12,'Bold')[1:11],
         rcartocolor::carto_pal(12,'Antique')[1:3])
names(pal) <- as.character(sort(unique(peak_ho_df$year)))

shapes <- c(rep(15,7), rep(19,7))
names(shapes) <- as.character(sort(unique(peak_ho_df$year)))
...
p <-
  ...
  scale_color_manual(name = "Annual timing of peak haul-out probability",
                     values = pal) +
  scale_x_continuous(
  ...
  scale_shape_manual(name = "Annual timing of peak haul-out probability",
                     values = shapes) +
  ...
  labs(caption = "predictions are shown for local solar noon and under mean weather conditions") +
...
seals. This aligns with findings from Thometz et al. [@thometz2021] who observed
a mean molting period of 119&pm;2 days and a relatively stable resting metabolic
rate for bearded seals during that time. While ribbon seals were not considered
in that study, spotted and ringed seals underwent molt periods of just 33&pm;4
and 28&pm;6 days and had increased resting metabolic rates.
...
[@hamilton2018; @perry2017]. In our study, ribbon seal haul-out behavior seemed to be the most
...
discussion in Supplemental Material [Exploring Insolation (Solar Radiation) as a
...
covariates are unaccounted for [@conn2023]. We recommend that future abundance
...
```{r ho-probs, include = FALSE}
bearded_ho_prob <- bearded_newdata |> 
  filter(yday == 141, solar_hour == 12) |> 
  select(ho_prob,lower95,upper95)
ribbon_ho_prob <- ribbon_newdata |> 
  filter(yday == 141, solar_hour == 12) |> 
  select(age_sex,ho_prob,lower95,upper95)
spotted_ho_prob <- spotted_newdata |> 
  filter(yday == 141, solar_hour == 12) |> 
  select(age_sex,ho_prob,lower95,upper95)
```
...
suggest a haul-out probability on May 20 at local solar noon of `r bearded_ho_prob$ho_prob[[1]]` 
(95% CI: `r bearded_ho_prob$lower95[[1]]` -- `r bearded_ho_prob$upper95[[1]]`)
for bearded seals across all age and sex classes, 
`r ribbon_ho_prob$ho_prob[[2]]` 
(95% CI: `r ribbon_ho_prob$lower95[[2]]` -- `r ribbon_ho_prob$upper95[[2]]`) for
adult male ribbon seals, `r ribbon_ho_prob$ho_prob[[1]]` 
(95% CI: `r ribbon_ho_prob$lower95[[1]]` -- `r ribbon_ho_prob$upper95[[1]]`) for
adult female ribbon seals, `r spotted_ho_prob$ho_prob[[2]]` 
(95% CI: `r spotted_ho_prob$lower95[[2]]` -- `r spotted_ho_prob$upper95[[2]]`) for
adult male spotted seals, and `r spotted_ho_prob$ho_prob[[1]]` 
(95% CI: `r spotted_ho_prob$lower95[[1]]` -- `r spotted_ho_prob$upper95[[1]]`) for
...
a future study (see, e.g., @lindsay2021).
...
GitHub (https://github.com/noaa-afsc/berchuksealsHauloutFits) and archived at 
Zenodo.
...
of Naval Research (No. N00014-16-1-3019). ADFG and NSB field work was covered by
...
```{r deploymentTable, include=TRUE, tab.cap="The  timing, location, and institutions responsible for the bio-logger deployments used in this study along with research permits and any associated publications."}
library(gt)
...
parameter. This
parameter is described as _"the amount of solar radiation (also known as shortwave radiation)
...
instrument used for measuring solar radiation) at the surface"_ 
...
(https://CRAN.R-project.org/package=reticulate) allowed interaction with Python.
...
with the ERA5 _surface solar radiation downwards_ (`era_ssrd_watts`) parameter. As
with other reanalysis values (from NARR) in the manuscript, the `era-ssrd-watts`
...
matched the formula used in the manuscript for ribbon seals. The `s(speno, bs = "re")` 
term is the smooth term for the random effect. All other predictors were
the same. 

```{r bam-m1-ribbon, include=TRUE, echo = TRUE}
m1_ribbon <- mgcv::bam(
...
```{r lag1-ribbon, include=FALSE}
lag1_ribbon <- acf(resid(m1_ribbon), plot=FALSE)[1][[1]]
```

Note, the specification for _m1_ribbon_ here does not include
any AR1 structure for temporal autocorrelation. To include this, we needed to
provide a value for $\rho$ (or _rho_). We examined the autocorrelation within
the model and used the lag-1 value for $\rho$ .The value for lag-1 
autocorrelation was `r formatC(as.numeric(lag1_ribbon))` which is
rather high but not surprising. We then updated our model specification with
a value for $\rho$ as well as the `A1.start` argument which specifies (as either 
__TRUE__ or __FALSE__) the start point of each block.

```{r bam-m2-ribbon, include=TRUE, echo = TRUE}
m2_ribbon <- mgcv::bam(
  dry ~ age_sex + s(speno, bs = "re") + 
    sin1 + cos1 + sin2 + cos2 + sin3 + cos3 + 
    poly(day, 3, raw=TRUE) + 
...
```{r bam-ssrd-ribbon, include=TRUE, echo = TRUE}
...
ssrd_solarhour_plot <- ggplot(data = pts_ssrd_sf, aes(x=solar_hour, y=value)) + 
  geom_point(size = 0.5, alpha = 0.3, color = "#edd9a3") + 
...
`r AIC(m2_bearded,m2_ssrd_bearded)['m2_ssrd_bearded','AIC']`. The ribbon seal model matching the specification from the manuscript resulted in 
...
`r AIC(m2_ribbon,m2_ssrd_ribbon)['m2_ssrd_ribbon','AIC']`. The spotted seal model matching the specification from the manuscript resulted in 
...
```{r ssrd-prediction-heatmap, include = TRUE, fig.asp=0.9, fig.cap="Predicted haul-out probability of ribbon seals from 15 March to 30 June for each age and sex class used in the model. In this model, solar radiation was used in lieu of hour of day. The apparent seasonal progression with subadults hauling out earlier in the season followed by adult males and, then, adult females is still notable although maybe not as clear. Predictions for young of the year still show their transition from newly weaned pups resting on the ice to more in-water activities. The overall pattern is in agreement with a one-sided view of Figure 7 where maximum solar radiation is equivalent to local solar noon."}
r <- ribbon_newdata |> 
...
p <- ggplot(plot_df, aes(day, ssrd_quants, fill = fit, color = fit)) +
  geom_tile(linewidth = 0.15) +
  scico::scale_fill_scico(palette = "navia", direction = 1,
                          begin = 0.1, end = 1,
...
p <- p + scale_x_continuous(breaks = c(5,10,15,20,25)) +
  scale_y_discrete(breaks = c("20%","60%","95%"), labels = c("15%","50%","100%")) +
  coord_cartesian(expand=FALSE)
p <- p + theme(panel.spacing.x = unit(0.1, "lines"),
               panel.spacing.y = unit(0.2, "lines"),
               legend.position = "bottom",
               strip.background = element_rect(colour="white"),
               axis.ticks=element_blank()) +
  xlab("day of month") + ylab("% of max solar radiation") +
  ggtitle("Solar radiation as a predictor of ribbon seal haul-out probability",
          subtitle = "Increased haul-out probability at higher levels of solar
          radiation is consistent with maximum predicted haul-out at solar noon")
p
```

```{r ssrd-ribbonPredSE,fig.asp = 0.7, include = TRUE, fig.cap="Seasonal variability in haul-out probability and the associated 95\\% confidence intervals (shaded area) for ribbon seals. In this model predictions are shown for low, medium, and high values of solar radiation (as percentages of the maximum value observed) in lieu of local solar hour. There's general agreement in the overall seasonal patterns between the two approaches but sublte differences in shape and extent of the confidence limits were present (see Figure S2 for comparisons)."}

ssrd_facet_labels <- c(
  '35%' = 'Low Solar Radiation (35% of max value)',
  '65%' = 'Medium Solar Radiation (65% of max value)',
  '100%' = 'High Solar Radiation (100% of max value)'
) |> as_tibble(rownames = 'ssrd_facet') |> 
  mutate(value = forcats::fct(value,levels = value))
...
plot_df <- ribbon_ssrd_predict %>%
  ...
  filter(!month %in% c("Jul","Aug"),
         lubridate::yday(date) >73,
         ssrd_quants %in% c("35%","65%","100%")) |> 
  inner_join(ssrd_facet_labels, by = join_by(ssrd_quants == ssrd_facet)) |> 
  rename(ssrd_facet_label = value)

ssrd_ribbon_predse <- ggplot(plot_df, aes(date,fit)) +
  geom_ribbon(aes(ymin=fit-(1.96*se.fit),ymax=fit + (1.96*se.fit)), 
              fill = species_colors['Ribbon seal'], alpha = 0.3) +
  geom_line(aes(date,fit),
            color = species_colors['Ribbon seal']) +
  facet_grid(age_sex ~ ssrd_facet_label,
             scales = "free_x",
                    space = "free_x",
                    labeller = labeller(
                      age_sex = age_sex_labels,
                      ssrd_facet_label = label_wrap_gen(23)
                    )) +
  scale_y_continuous(breaks = c(0.2,0.4,0.6,0.8), labels = c("0.2","0.4", "0.6", "0.8"), limits = c(0,1)) +
  xlab("day of year") +
  ylab("haul-out probability") +
  labs(title = "Solar radiation as a predictor of haul-out probability (with uncertainty)",
       subtitle = "Predictions and associated 95% confidence intervals for three **solar radiation** quantile categories show similar seasonal patterns to those based on local solar hour") +
  theme(legend.position = "none")
...
<!-- - ear5 solar radiation has potential as an informative covariate that is a more -->
<!-- directly linked with seal behavior and physiology -->
<!-- - the unimodal distribution and correlation with hour of day also demonstrates -->
<!-- it could be considered a replacement for hour of day; though, with increased -->
<!-- variability given the impact of cloud cover and other weather parameters -->
<!-- - despite the reduced number of parameters, AIC comparison for all species -->
<!-- still preferred the model with local solar hour.  -->
<!-- - there is some a slight skewness present in the predicted distributions with -->
<!-- higher predictions in the morning compared to the late afternoon; this isn't -->
<!-- captured with solar radiation alone -->

Solar radiation has potential as an informative
covariate in pinniped haul-out models that can be directly linked to seal
physiology and expected behavioral changes. The ERA5's _surface solar radiation downwards_
values aligned with hour of day and maximum values
occurred at or just after local solar noon. This highlighted the informative
potential for this approach. However, despite an overall reduction in the
total number of parameters and degrees of freedom, AIC comparison still favored
the models for each species that included hour of day as a Fourier series. This
analysis was not intended to be a full comparison -- we simply want to 
demonstrate the potential and inspire further investigation -- but, there are
three possibilities that might explain the preference for hour of day. 

First,
there are a broad range of solar radiation values represented for each hour of the
day. Cloud cover, fog, and precipitation all reduce downward solar radiation at
the surface and we might expect this to impact haul-out probability. However, the photoperiod and the timing of sunrise and sunset are
...
physiological cycles and energetic needs and higher solar radiation may not be
a consistently driving influence on seals. Increased energy from the sun may
be important during molt but less so during pupping and breeding periods. Second,
the timing and duration of haul-out behavior may also be influenced by diel
patterns in weather (e.g. lower winds in the morning) or ecosystem dynamics (e.g.
prey availability) that lead to a skewness in the distribution of haul-out 
...
Again, we want to acknowledge Anthony Fisbach for the suggestion during the
