...
notion being that seals are directly responding to changes in solar radiation
...
Because of these reasons, we spent some time considering and exploring this
possibility for our model and the analysis presented in this manuscript. A key
drawback to reliance on solar insolation, in our minds, is that we would lose
insight regarding potential crepuscular patterns -- insolation does not 
...
```{r, include=FALSE}
ssrd_dataset <- readRDS(file = here::here('data/ssrd_dataset.rds'))
...
```{r era5-ssrd-plot, fig.asp = 0.7, include=TRUE, fig.cap="Downward surface solar radiation estimates from the ERA5 climate reanalysis for 5000 random points within the study area between 2005 and 2021. Solar radiation values are presented in Watts per meter^2 and the smoothed line highlights the strong diel pattern."}
ssrd_solarhour_plot <- ggplot(data = pts_ssrd_sf, aes(x=solar_hour, y=value)) + 
  ...
  labs(title = "Diel Pattern of Solar Radiation Values from ERA5 Reanalysis",
       subtitle = "A random sample of hourly ERA5 downward surface solar radiation values for random points within the study area confirms the expected unimodal relationship with local solar hour.",
