---
title: "Spring haul-out behavior of seals in the Bering and Chukchi Seas: implications for abundance estimation"
preprint: false
author: 
  - name: Josh M. London
...
abstract: >
  ...
  activities occur in spring (April through June) as sea ice begins to melt and
  ...
  forecast to decrease. Robust estimates of seal population abundance are needed
  to properly monitor the impacts of these changes over time. Aerial surveys of
  seals on ice are an efficient method for counting seals but must be paired
  with estimates of the proportion of seals out of the water to derive
  population abundance. In this paper, we use hourly percent-dry data from
  satellite-linked bio-loggers deployed between 2005 and 2021 to quantify the
  proportion of seals hauled out on ice. This information is needed to
  accurately estimate abundance from aerial survey counts of ice-associated
  seals (i.e., to correct for the proportion of animals that are in the water,
  and so are not counted, while surveys are conducted). In addition to providing
  essential data for survey 'availability' calculations, our analysis also
  provides insights into the seasonal timing and environmental factors affecting
  haul-out behavior by ice-associated seals. We specifically focused on bearded
  (_Erignathus barbatus_), ribbon (_Histriophoca fasciata_), and spotted seals
  (_Phoca largha_) in the Bering and Chukchi seas. Because ringed seals (_Phoca
  (pusa) hispida_) can be out of the water but hidden from view in snow lairs
  analysis of their 'availability' to surveys requires special consideration;
  therefore, they were not included in this analysis. Using generalized linear
  mixed pseudo-models to properly account for temporal autocorrelation, we fit
  models with covariates of interest (e.g., day-of-year, solar hour, age-sex
  class, wind speed, barometric pressure, temperature, precipitation) to examine
  their ability to explain variation in haul-out probability. We found evidence
  for strong diel and within-season patterns in haul-out behavior, as well as
  strong weather effects (particularly wind and temperature). In general, seals
  were more likely to haul out on ice in the middle of the day and when wind
  speed was low and temperatures were higher. Haul-out probability increased
  through March and April, peaking in May and early June before declining again.
  The timing and frequency of haul-out events also varied based on species and
  age and sex class. For ribbon and spotted seals, models with year effects were
  highly supported, indicating that the timing and magnitude of haul-out
  behavior varied among years. However, we did not find broad evidence that
  haul-out timing was linked to annual sea-ice extent. Our analysis emphasizes
  the importance of accounting for seasonal and temporal variation in haul-out
  behavior, as well as associated environmental covariates, when interpreting
...
make_plot_namer <- function(prefix="Figure-", fig_n=1L, width=2){
  function(x){
    x2 <- file.path(dirname(x), 
                    paste0(prefix, 
                           formatC(fig_n, width=width, flag="0"),
                           ".", 
                           tools::file_ext(x)))
    fig_n <<- fig_n + 1L
...
knitr::opts_chunk$set(fig.process = make_plot_namer(width=3))
...
shown to influence walrus haul-out behavior [@udevitz2009] and we expected weather
...
Supplemental Information Table [S1](#s1)). NOAA-led research was conducted under the
...
p <- ggplot() +
  ...
  scale_fill_manual(values = species_colors,
                    aesthetics = "fill", 
                    name = "species") + 
  labs(caption = "<span style = 'color: #A26E7C;'><span style = 'font-family: \"Font Awesome 6 Free Regular\";'>&#xf0c8;</span> spatial extent of aerial survey effort in US and Russian waters</span><br />world land vector data from naturalearthdata.com") +
...
(GLMPMs; Ver Hoef, London & Boveng [-@verhoef2010a]) to model variation in
...
[@verhoef2010a] to implement GLMPMs. We explored two different model
...
ggplot() +
  ...
  facet_grid( ~ species) +
  labs(
    caption = "One hexagonal cell represents 50 km^2^<br />world land vector data from naturalearthdata.com"
  ) +
  theme(
...
increasing transmission rates (see Boveng & Cameron [-@boveng2013])). All data after the first
...
p <- ggplot(plot_df, aes(day,solar_hour,fill = ho_prob, color = ho_prob)) +
  ...
  scico::scale_fill_scico(palette = "navia", direction = 1,
                          begin = 0.1, end = 1,
                       name = "Ribbon seal haul-out probability", 
...
interactions between day and year for only spotted seals (_day:year_,
...
`r spotted_year_type3$day_year$Prob.F`; _day^2^:year_,
...
# see https://davidmathlogic.com/colorblind/#%23A6CEE3-%233385BB-%2384BF96-%23004D40-%23F3E587-%23F57C7C-%23D81B60-%23FBB268-%23FE8D19-%23DE9E83-%239D7BBA-%23977899-%237F9D55-%23B15928
# for simulation of colorblind scenarios

pal <- c("#A6CEE3","#3385BB","#84BF96","#004D40","#F3E587","#F57C7C","#D81B60",
         "#FBB268","#FE8D19","#DE9E83","#9D7BBA", "#977899","#7F9D55","#B15928")
...
and males [@reder2003; @thompson1987; @kirkman2003]. Also of note is the early
...
[@hamilton2018; @perry2017]. In our study, ribbon seal haul-out behavior was notably
...
appeared to impact body condition of ribbon and spotted seals [@boveng2020], so
...
covariates are unaccounted for (see @conn2023 for discussion about the
...
originally published as a pre-print at bioRxiv (@london2022;
https://doi.org/10.1101/2022.04.07.487572). All data used in the study and code
are available on GitHub (https://github.com/noaa-afsc/berchukseals-haulout) and
...
the United States Animal Telemetry Network (https://portal.atn.ioos.us/; 
https://doi.org/10.24431/rw1k8er), archived at Movebank (see
...
(https://github.com/noaa-afsc/berchuksealsHauloutFits) and archived at Zenodo
(https://doi.org/10.5281/zenodo.10056308).
...
bearded_predse <- ggplot(plot_df, aes(date,ho_prob)) +
  geom_ribbon(aes(ymin=lower95,ymax=upper95, fill = age_sex), 
              fill = species_colors['Bearded seal'], alpha = 0.3) +
  geom_line(aes(date,ho_prob, color = species)) +
  scale_color_manual(name = 'Predicted haul-out probability',
                     values = "#E58606",
...
(ref:ribbonPredSEcaption) **Seasonal variability in haul-out probability and the associated 95\% confidence intervals (shaded area) for ribbon seals.** \linebreak Model predictions are shown for three local solar hours (07:00, 12:00, and 17:00). Weather covariate values in the prediction were based on a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season. Age and sex classes are separated to allow comparisons.

```{r ribbonPredSE,fig.asp = 0.7, include = TRUE, fig.cap="(ref:ribbonPredSEcaption)"}
plot_df <- ribbon_newdata %>%
  mutate(date = lubridate::as_date(yday, origin = "2015-01-01"),
         month = lubridate::month(date,label = TRUE),
         day = lubridate::day(date)) %>%
...
(ref:spottedPredSEcaption) **Seasonal variability in haul-out probability and the associated 95\% confidence intervals (shaded area) for spotted seals.** \linebreak Model predictions are shown for three local solar hours (07:00, 12:00, and 17:00). Weather covariate values in the prediction were based on a simple generalized additive model for each weather covariate with smooth terms for day-of-year and solar hour to account for anticipated variability within a day over the season. Age and sex classes are separated to allow comparisons.

```{r spottedPredSE,fig.asp = 0.7, include = TRUE, fig.cap="(ref:spottedPredSEcaption)"}
plot_df <- spotted_newdata %>%
  mutate(date = lubridate::as_date(yday, origin = "2015-01-01"),
         month = lubridate::month(date,label = TRUE),
         day = lubridate::day(date)) %>%
...
(ref:era5ssrdcaption) **Diel Pattern of Solar Radiation Values from ERA5 Reanalysis.** \linebreak Downward surface solar radiation estimates from the ERA5 climate reanalysis for 5000 random points within the study area between 2005 and 2021. Solar radiation values are presented in Watts per square-meter and the smoothed line highlights the strong diel pattern.

```{r era5-ssrd-plot, fig.asp = 0.75, include=TRUE, fig.cap="(ref:era5ssrdcaption)"}

pts_ssrd_sf <- readRDS(here::here('data/random_ssrd_pts.rds')) |> 
  mutate(era5_value = "downward surface solar radiation")

ssrd_solarhour_plot <- ggplot(data = pts_ssrd_sf, aes(x=solar_hour, y=value)) + 
  geom_point(aes(color = era5_value), size = 0.8, alpha = 0.3) +
  scale_color_manual(name = 'ERA5 Reanalysis',
                     values = "#edd9a3",
                     guide = guide_legend(title.position = "top",
                                          title.hjust = 0,
                                          direction = "horizontal",
  ...
  labs(caption = "solar radiation values are presented in Watts per meter^2") +
  theme(legend.position = "top",
        legend.justification = "left")
...
ssrd_ribbon_predse <- ggplot(plot_df, aes(date,fit)) +
  geom_ribbon(aes(ymin=fit-(1.96*se.fit),ymax=fit + (1.96*se.fit)), 
              fill = species_colors['Ribbon seal'], alpha = 0.3) +
  geom_line(aes(date,fit, color = species)) +
  scale_color_manual(name = 'Predicted haul-out probability',
                     values = "#5D69B1",
                     guide = guide_legend(title.position = "top",
                                          title.hjust = 0,
...
changes. The ERA5's _surface solar radiation downwards_ values aligned with hour
