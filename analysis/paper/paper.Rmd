...
include only records from 1 March to 15 July between 
...
seal_hours_tbl <- analysis_data %>% 
  tibble::as_tibble() %>% 
  dplyr::mutate(yday = lubridate::yday(haulout_dt)) %>% 
  dplyr::left_join(yday_groups) %>% 
  dplyr::group_by(species, col_group, row_group) %>% 
  dplyr::count()
```
