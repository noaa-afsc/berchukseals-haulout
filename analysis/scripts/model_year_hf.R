...
HO_ribbon %>% filter(across(everything(), ~ is.na(.x))) %>% nrow()

fit_ribbon_year <- glmmLDTS(fixed.formula = dry ~ age_sex + sin1 + cos1 + sin2 + cos2 + sin3 + cos3 + day + day2+ day3 + temp2 + wind + pressure + precip + wind*temp2 +
...
save(fit_ribbon_year, file = here::here('data/fit_ribbon_year.Rdata'))
